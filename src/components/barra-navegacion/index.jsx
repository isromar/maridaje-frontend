    ...
    if (usuario === "" && (password !== "" || password === '')) {
      ...
      if (data && data["hydra:member"].length > 0) {
        ...
        if (options[0].label === usuario) {
          const decryptedPassword = EncryptionUtils.decryptPassword(options[0].password);
          if (password === decryptedPassword) {
            ...
            mostrarMensaje("Error de acceso", "La bodega no está registrada o la contraseña es incorrecta", "error");
            setAcceso(false);
            setTimeout(() => {
              Swal.close();
            }, 4000);
          ...
          mostrarMensaje("Error de acceso", "La bodega no está registrada", "error");
          setAcceso(false);
          setTimeout(() => {
            Swal.close();
          }, 4000);
